...
import { ScanLine, CheckCircle, X, ChefHat, Pencil, Minus, MessageSquare, Send, Bot, User, Settings2, Trash2, Plus, Bookmark, Flame } from 'lucide-react';
...
export default function ScanPage() {
  ...
  const handleScanResult = (result) => {
    if (result && result.detected_food) {
      // Capitalize first letter
      const foodName = result.detected_food.charAt(0).toUpperCase() + result.detected_food.slice(1);
      
      // Add to basket if not there
      if (!selectedIngredients.includes(foodName)) {
        setSelectedIngredients(prev => [...prev, foodName]);
      }
    }
  };
  ...
  const openChatbot = () => {
    ...
    if (messages.length === 0) {
      setMessages([{ 
        role: 'ai', 
        text: `I see you have scanned and selected ${selectedIngredients.length} items! Before I generate your recipe, please let me know: Do you have any dietary problems, health issues, or specific household items I should consider?` 
      }]);
    }
  };
    ...
    <div className="max-w-7xl mx-auto animate-fade-in space-y-8 relative">
      <header className="mb-8 flex items-center gap-4">
        ...
        <div>
            ...
            <p className="text-gray-500 dark:text-gray-400">Upload a photo to detect ingredients, add pantry staples, and generate a recipe.</p>
        </div>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        
        {/* LEFT COLUMN: Scanner & Pantry */}
        <div className="lg:col-span-5 space-y-6">
          
          {/* THE SCANNER */}
          <div className="bg-white dark:bg-dark-card p-1 rounded-2xl border border-gray-200 dark:border-dark-border shadow-lg transition-colors">
            <CameraUpload 
              appliances={[]} 
              constraints={[]} 
          ...
          <div className="bg-white dark:bg-dark-card p-6 rounded-2xl border border-gray-200 dark:border-dark-border shadow-sm transition-colors">
             <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Add Pantry Staples</h2>
             </div>
          ...
          {/* YOUR BASKET */}
          <div className="bg-white dark:bg-dark-card p-6 rounded-2xl border border-gray-200 dark:border-dark-border shadow-sm transition-colors">
             <h2 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">Your Basket</h2>
             <div className="flex flex-wrap gap-2 min-h-[50px] p-4 bg-gray-50 dark:bg-dark-bg rounded-xl border border-dashed border-gray-200 dark:border-gray-700">
                 {selectedIngredients.length > 0 ? (
                     ...
                     <span className="text-sm text-gray-400 italic flex items-center w-full justify-center">Scan an image above to add an ingredient.</span>
                 )}
             </div>
          </div>
          ...
          >
              <MessageSquare size={20} /> Generate Recipe (AI)
          </button>
        </div>
        ...
        <div className="lg:col-span-7">
           {!generatedRecipe ? (
             <div className="bg-white dark:bg-dark-card border-2 border-dashed border-gray-200 dark:border-dark-border rounded-3xl p-12 flex flex-col items-center justify-center text-center h-full min-h-[500px] transition-colors">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">Scan to Start</h2>
                <p className="text-gray-500 dark:text-gray-400 max-w-md">
                  Upload an image of an ingredient on the left, add some pantry basics, and let our AI create a perfect recipe for you.
                </p>
             </div>
           ) : (
            ...
            <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-dark-card">
              <div className="relative flex items-center">
                <input 
                  ...
                  className="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-full pl-6 pr-14 py-4 text-gray-900 dark:text-white focus:border-brand-green outline-none transition-colors"
